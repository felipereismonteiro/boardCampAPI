...
export default async function gameMiddleware(req, res, next) {
    try {
        ...
        const categorie = await connection.query("SELECT * FROM categories WHERE id=$1", [categoryId])
        const nameFounded = await connection.query("SELECT * FROM games WHERE name=$1", [name]);
